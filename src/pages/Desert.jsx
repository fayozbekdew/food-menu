...
const Desert = () => {
    const {data: foods, error, loading} = useFetch('https://my-json-server.typicode.com/fayozbekdew/food-db/recipes?region=desert')
    return (
      ...
      {foods && foods.map((food) => {
          ...
          <Link to={`/desert/${food.id}`} className='mx-auto rounded-[5px] w-full max-w-[100px] mb-3 border-solid border-2 border-black hover:bg-slate-50'>Read More</Link>
      </div>
          )
      })}
      </ul>
